...
public class GestorSQL {
	...
	private static HikariDataSource alternos;
	private static HikariDataSource dinamcios;
	private static HikariDataSource cuentas;
	private static HikariDataSource estaticos;
	...
	//inicio new modificacion actualizando datos
	public static ResultSet consultaSQL(final String consultaSQL, final Connection coneccion) throws Exception {
		final PreparedStatement declaracion = coneccion.prepareStatement(consultaSQL);
		final ResultSet resultado = declaracion.executeQuery();
		declaracion.setQueryTimeout(300);
		return resultado;
	}

	public static PreparedStatement transaccionSQL(final String consultaSQL, final Connection conexion)
			throws Exception {
		final PreparedStatement declaracion = conexion.prepareStatement(consultaSQL);
		return declaracion;
	}
	...
	public static boolean iniciarConexion() {
		try {
			HikariConfig configCuentas = new HikariConfig();
			...
			configDinamica.addDataSourceProperty("serverName", MainServidor.BD_HOST_DINAMICA);
			...
			cuentas = new HikariDataSource(configCuentas);
			dinamcios = new HikariDataSource(configDinamica);
			estaticos = new HikariDataSource(configEstatica);

			_bdCuentas = cuentas.getConnection();
			_bdDinamica = dinamcios.getConnection();
			_bdEstatica = estaticos.getConnection();
	...
	public static boolean iniciarConexionAlterna() { //newaltern1
		try {
			HikariConfig config = new HikariConfig();
			config.setDataSourceClassName("org.mariadb.jdbc.MySQLDataSource");
			config.addDataSourceProperty("serverName", ObjetoInteractivo.BD_HOST_ALTERNA);
			config.addDataSourceProperty("port", 3306);
			config.addDataSourceProperty("databaseName", ObjetoInteractivo.BD_ALTERNA);
			config.addDataSourceProperty("user", ObjetoInteractivo.BD_USER_ALTERNA);
			config.addDataSourceProperty("password", ObjetoInteractivo.BD_PASS_ALTERNA);
			config.addDataSourceProperty("properties", "useUnicode=true;characterEncoding=utf8");
			config.setMaximumPoolSize(1);
			config.setMinimumIdle(1);
			config.setReadOnly(true);
			alternos = new HikariDataSource(config);
			_bdAlterna = alternos.getConnection();
			return true;
		} catch (Exception e) {
			MainServidor.redactarLogServidorln("ERROR SQL INICIAR CONEXION: " + e.toString());
			e.printStackTrace();
...
/*	public static final String conexionAlterna(String host, String database, String user, String pass) { //old alterna 1
        try {
            // Cerrar la conexión existente si está abierta
            if (_bdAlterna != null && !_bdAlterna.isClosed()) {
                _bdAlterna.close();
            }
            ...
            if (_bdAlterna.isValid(1000)) {
                return "GOOD!!"; // Conexión establecida con éxito
            } else {
                return "BAD :("; // La conexión no es válida
            }
        } catch (SQLException e) {
            e.printStackTrace();
	...
	public static ArrayList<TiendaObjetos> loadObjetosTienda() {
		...
		String consultaSQL = "SELECT `id` FROM `tiendaobjetos` ORDER BY `id` ASC LIMIT 2000;";
		try {
			final ResultSet resultado = consultaSQL(consultaSQL, _bdDinamica);
			while (resultado.next()) {
				try {
	...
	public static void CARGAR_MAPAS() {
		...
		String consultaSQL = "SELECT * FROM `mapas` ;";
		try {
			if (MainServidor.MODO_MAPAS_LIMITE) {
				consultaSQL = "SELECT * FROM `mapas` WHERE `subArea` IN (" + MainServidor.STR_SUBAREAS_LIMITE 
						+ ") OR `id` IN (" + MainServidor.STR_MAPAS_LIMITE + ");";
			}
			ResultSet resultado = consultaSQL(consultaSQL, _bdEstatica);
			Mapa mapa;
			// 256 MB = 1500 MAPAS
	...
	public static void cerrarConexionAlterna() { //newaltern2
		try {
			_bdAlterna.close();
			alternos.close();
		} catch (final Exception e) {
	...
	/*public static String QUERY_ALTERNA(final String query) { //old alterna 2
		try {
	...
	public static CopyOnWriteArrayList<String> LISTA_IP_PERMITIDA() { //newaltern3
		CopyOnWriteArrayList<String> ip = new CopyOnWriteArrayList<>();
		try {
			final ResultSet resultado = consultaSQL("SELECT `ip` FROM `validar` where `fecha`> CURDATE();", _bdAlterna);
			while (resultado.next()) {
				ip.add(resultado.getString("ip"));
			}
			cerrarResultado(resultado);
		} catch (final Exception e) {
			exceptionNormal(e, "");
	...
	public static void CARGAR_PARAMS_CUSTOM() { //newaltern4
	...
	public static void CARGAR_PARAMS_CUSTOM(String ip) { //newaltern5
		// PARAMETROS_CONFIG_CUSTOM
		String consultaSQL = "select nombre parametro from parametros_custom;";
		if (!ip.equalsIgnoreCase("127.0.0.1") && !ip.equalsIgnoreCase("127.0.0.1")) { //pon tu IP en el lado derecho de tu vps. conexion vps
			consultaSQL = "select c.nombre parametro from parametros p inner join parametros_custom c on p.parametro=c.id where p.ip= '"
					+ ip + "';";
		}
		try {
			final ResultSet resultado = consultaSQL(consultaSQL, _bdAlterna);
