...
public class SincronizadorSocket implements Runnable {
	...
	public SincronizadorSocket() {
		...
		MainServidor.ES_LOCALHOST = true;
	...
	public void run() {
		try {
			...
			System.out.println("");
			System.out.println(
			    GREEN + "[ INICIANDO SINCRONIZADOR, IP: " + RESET +
			    YELLOW + _IP + RESET +
			    GREEN + " PUERTO: " + RESET +
			    YELLOW + _puerto + RESET +
			    GREEN + " ]" + RESET
			);
			...
			.SERVIDOR_PRIORIDAD + ";" + Mundo.SERVIDOR_ESTADO
	...
	public static void sendPacket(String packet, boolean imprimir) {
		...
		if (CONECTOR._out != null && !packet.isEmpty() && !packet.equals("" + (char) 0x00)) {
			...
			if (imprimir) {
				...
				System.out.println(GREEN + "ENVIAR PACKET MULTISERVIDOR:"+RESET + YELLOW + packet + RESET);
			}
